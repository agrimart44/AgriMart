...
def make_predictions(model, data, weeks=4):
    """
    Make predictions for weekly market prices

    Parameters:
    model: trained prediction model
    data: DataFrame with weekly market prices
    weeks: number of weeks to forecast

    Returns:
    tuple: (predictions_list, confidence_intervals)
    """
    # Get the last date from data
    last_date = data.index[-1]

    # Calculate next collection date
    # If last date was a Tuesday, next prediction will be for next Tuesday, etc.
    collection_day = last_date.weekday()

    # Prepare exogenous variables
    last_rolling_mean = data['rolling_mean'].iloc[-1]
    last_rolling_std = data['rolling_std'].iloc[-1]

    # Create future exog data (one per week)
    ...
    # Generate future dates maintaining the same day of week
    prediction_dates = []
    for i in range(weeks):
        # Add exactly 7 days each time to maintain the collection day
...
# """
# # If your last data point is from January 30, 2025 (a Thursday)
# # and you want 4 weeks of predictions:
# predictions, intervals = make_predictions(model, data, weeks=4)
#
# # This will give predictions for:
