...
def extract_vegetable_wholesale_prices(pdf_path):
    ...
    with open(pdf_path, 'rb') as file:
        ...
        vegetables_to_filter = ['Brinjal', 'Cabbage', 'Carrot', 'Lime', 'Pumpkin', 'Snake gourd', 'Tomato']
...
class VegetablePriceAnalyzer:
    def __init__(self, z_score_threshold=3):
        ...
        self.vegetables = ['Brinjal', 'Cabbage', 'Carrot', 'Lime', 'Pumpkin', 'Snake gourd', 'Tomato']
    ...
    def _prepare_data(self, df):
        ...
        for veg in self.vegetables:
            try:
                ...
                if len(veg_df) > 0:
                    # Convert 'today' column to numeric and drop NaN values
                    ...
                    if len(veg_df) > 0:
                        ...
                        # Retain both 'Price' and 'Date' columns
    ...
    def find_optimal_parameters(self, train_data):
        ...
        try:
            ...
            logging.info(f"✅ Optimal parameters found using auto_arima: SARIMA{order}x{seasonal_order}")
            return order, seasonal_order

        except Exception as e:
            logging.error(f"Error finding optimal parameters using auto_arima: {e}")

            # Fallback to default parameters if auto_arima fails
            logging.warning("⚠️ Falling back to default parameters: SARIMA(1, 1, 1)x(1, 1, 1, 7)")
    ...
    def train_sarima_model(self, data, order, seasonal_order):
        ...
        try:
            ...
            logging.error(f"Error training complex model: {e}")
    ...
    def make_predictions(self, model, data, days=7):
        """
        Make predictions for the next 7 days of market prices using the system date.
        """
        # Get the last rolling mean and rolling standard deviation from the data
        last_rolling_mean = data['rolling_mean'].iloc[-1]
        last_rolling_std = data['rolling_std'].iloc[-1]

        # Create a DataFrame for future exogenous variables (rolling_mean and rolling_std)
        ...
        # Generate prediction dates starting from the system date
        start_date = datetime.now().date()  # Capture the current system date once
        prediction_dates = [start_date + timedelta(days=i + 1) for i in range(days)]

        # Combine prediction dates and forecasted values into a list of tuples
    ...
    def analyze_commodity(self, data, commodity_name):
        ...
        # Check if the data exists and has enough points
        if price_series is None or len(price_series) < 14:
            logging.warning(f"⚠️ Insufficient data for analysis: {commodity_name}")
            return

        # Ensure price_series is a DataFrame
        ...
        # Check for missing 'Date' values after conversion
        missing_dates = price_series[price_series['Date'].isna()]
        if not missing_dates.empty:
            logging.warning(f"⚠️ Some 'Date' values could not be converted to datetime in {commodity_name}")
            print("Missing dates:\n", missing_dates)
            return
        ...
        try:
            price_series = price_series.asfreq('D')  # Set daily frequency
            price_series.fillna(method='ffill', inplace=True)  # Forward-fill missing values
        except Exception as e:
        ...
        # Log the ranges of training and testing data
    ...
    def main(self):
        ...
        combined_df = pd.concat([
            ...
            vegetable_prices_100, vegetable_prices_101, vegetable_prices_102, vegetable_prices_105,
            vegetable_prices_106,
            vegetable_prices_107,
            vegetable_prices_108, vegetable_prices_109, vegetable_prices_112, vegetable_prices_113,
            vegetable_prices_114,
            vegetable_prices_115,
            vegetable_prices_116, vegetable_prices_119,
            vegetable_prices_120, vegetable_prices_121, vegetable_prices_122, vegetable_prices_126,
            vegetable_prices_127,
            vegetable_prices_128, vegetable_prices_129, vegetable_prices_130, vegetable_prices_133,
            vegetable_prices_134,
            vegetable_prices_135, vegetable_prices_136, vegetable_prices_137, vegetable_prices_140,
            vegetable_prices_141,
            vegetable_prices_142, vegetable_prices_143, vegetable_prices_144, vegetable_prices_147,
            vegetable_prices_149,
            vegetable_prices_150, vegetable_prices_151, vegetable_prices_154, vegetable_prices_155,
            vegetable_prices_156,
            vegetable_prices_157, vegetable_prices_158, vegetable_prices_161, vegetable_prices_162,
