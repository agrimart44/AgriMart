...
def extract_vegetable_wholesale_prices(pdf_path):
    ...
    with open(pdf_path, 'rb') as file:
        ...
        df = df[df['Vegetable'].str.strip().isin(['Brinjal'])]
...
class VegetablePriceAnalyzer:
        ...
        for param in pdq:
            for seasonal_param in seasonal_pdq:
                try:
                    ...
                    logging.error(f"Error fitting model with params {param}, {seasonal_param}: {e}")
                    continue
    ...
    def make_predictions(self, model, data, weeks=4):
        """
        Make predictions for weekly market prices.
        """
        ...
        future_exog = pd.DataFrame({
            'rolling_mean': [last_rolling_mean] * weeks,
            'rolling_std': [last_rolling_std] * weeks
        })

        forecast = model.forecast(steps=weeks, exog=future_exog)
        confidence_intervals = model.get_forecast(steps=weeks, exog=future_exog).conf_int()

        prediction_dates = []
        for i in range(weeks):
            next_date = datetime.now() + timedelta(days=(i + 1) * 7)
            prediction_dates.append(next_date.date())
    ...
    def analyze_commodity(self, data, commodity_name):
        ...
        if not price_series.index.is_monotonic_increasing:
            logging.error(f"❌ Date index is not monotonic for {commodity_name}")
            return
        ...
        print("Columns after setting index:", price_series.columns)
